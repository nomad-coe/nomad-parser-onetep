...
def test_basic(parser):
    ...
    assert sec_run.program_version == '4.4.1'

    sec_system = archive.section_run[0].section_system[0]
    assert sec_system.lattice_vectors[2][2].magnitude == approx(2.0108734e-09)
    assert sec_system.atom_labels[3] == 'Cl'
    assert sec_system.atom_positions[4][2].magnitude == approx(1.01078267e-09)

    sec_scc = sec_run.section_single_configuration_calculation[0]
    assert sec_scc.energy_total.magnitude == approx(-2.24380887e-16)
